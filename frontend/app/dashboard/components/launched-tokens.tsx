      ...
      <CardContent className="p-6">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold">
            Launched{" "}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-sky-400 to-blue-500">Tokens</span>
          </h2>
          <Link href="/launch">
        ...
        <div className="mt-2">
          ...
          <div className="flex gap-2">
            {chainDistribution.map((chain) => (
              <div key={chain.chain} className="flex-1 bg-black/40 rounded-lg p-2 text-center border border-white/10">
                <p className="text-sm font-medium">{chain.chain}</p>
                <p className="text-lg font-bold text-blue-400">{chain.percentage}%</p>
              </div>
            ))}
          </div>
        </div>
