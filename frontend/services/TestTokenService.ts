...
export const useTestTokenService = () => {
  ...
  const getContractAddress = useCallback(() => {
    ...
    const contractAddress = config[chainId as keyof typeof config]?.factory?.address;
    if (!contractAddress) {
      throw new Error(`Factory contract address not found for chain ID ${chainId}`);
    }
    return contractAddress;
  }, []);

  const testGetTokens = useCallback(async (options: GetTokensOptions = {}) => {
    try {
      if (!walletClient) {
      ...
      const tokensWithMetadata = await Promise.all(
        filteredTokens.map(async (sale) => {
          try {
            ...
            console.log(`Error fetching metadata for token ${sale.token}:`, error);
            return {
              token: sale.token,
              name: sale.name,
              creator: sale.creator,
  ...
  const testBuyToken = useCallback(async (
    tokenSale: TokenSale,
    amount: bigint
    ...
    try {
      ...
      if (error.code === 4001) {
        ...
        return { success: false, error: "Internal JSON-RPC error. Check your wallet balance." };
      }
  ...
  const testCreateToken = useCallback(async (
