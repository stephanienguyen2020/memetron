...
export const useTokenGeneratingService = () => {
  const generateTokenWithAI = async (input: string): Promise<TokenAIGeneratedDetails> => {
    try {
      const response = await generateTokenConcept(input);
      
      // Generate image using the description
      const { imageUrl, imageBase64 } = await generateImageFromPrompt(response.image_description);
  ...
  const generateImageFromPrompt = async (prompt: string): Promise<{ imageUrl: string, imageBase64: string }> => {
    if (!prompt.trim()) {
