...
import { TrendingUp, Trophy, XCircle, ArrowRight, Wallet, Target } from "lucide-react"
      ...
      <CardContent className="p-6">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold">
            My <span className="text-transparent bg-clip-text bg-gradient-to-r from-sky-400 to-blue-500">Bets</span>
          </h2>
          <Link href="/bets">
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="bg-black/40 rounded-lg p-4 border border-white/10">
            ...
            <p className="text-sm text-gray-400 mt-1">Value: {betStats.activeValue}</p>
          </div>
          <div className="bg-black/40 rounded-lg p-4 border border-white/10">
            <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-sm text-gray-400 mt-1">of {betStats.totalBets} total</p>
          </div>
          <div className="bg-black/40 rounded-lg p-4 border border-white/10">
            <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-sm text-gray-400 mt-1">
              Rate: {(100 - Number.parseFloat(betStats.winRate.slice(0, -1))).toFixed(1)}%
            </p>
          </div>
        </div>
        ...
        <div className="flex justify-between items-center p-4 bg-black/40 rounded-lg border border-white/10">
          <div>
            ...
            <p className="text-xl font-bold text-green-400">{betStats.totalWinnings}</p>
          </div>
          <div>
            <p className="text-sm text-gray-400">Avg Return</p>
            <p className="text-xl font-bold text-blue-400">{betStats.avgReturn}</p>
          </div>
          <div>
            <p className="text-sm text-gray-400">Win Rate</p>
            <p className="text-xl font-bold text-blue-400">{betStats.winRate}</p>
          </div>
        </div>
      </CardContent>
    </Card>
